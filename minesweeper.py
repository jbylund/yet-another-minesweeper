...
class Minesweeper_model:
    def __init__(self,icols,irows,ibombs):
        ...
        self.data = [ [ Minesweeper_square() for i in range(self.cols) ] for j in range(self.rows) ]
    ...
    def check_victory_condition(self):
        # game is over if (# of placed flags + explored squares) = # of squares
        if(self.placed_flags + self.explored_squares == self.cols*self.rows):
            return True
        else:
            return False
...
class Minesweeper_window(Gtk.Window,Minesweeper_model):
    ...
    def on_timeout(self):
        ...
        self.timer.set_label("Time: " + `game_time.seconds`)
        return True

    def update_flag_counter(self):
        self.flag_counter.set_label("Flags: " + `self.model.num_bombs - self.model.placed_flags` )
        return
        ...
        if(row < 0 or 
           ...
           self.model.data[row][col].flag):
            ...
            if(0 == self.model.data[row][col].neighboring_bombs):
                ...
                label = `self.model.data[row][col].neighboring_bombs`
...
win = Minesweeper_window(my_model)
